...
export const GET = withUserAuth(async (_auth, req) => {
  ...
  let query = db.from('rs_counterparties').select('*').order('company_name_ko', { ascending: true })
...
export const POST = withBrokerSchema(CounterpartyCreateSchema, async (body) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const db = supabase as any
  const { data, error } = await db.from('rs_counterparties').insert(body).select().single()
