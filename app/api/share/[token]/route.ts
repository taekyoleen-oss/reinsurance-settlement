...
export async function GET(
  ...
  try {
    ...
    if (!tokenRow) throw new NotFoundError('유효하지 않거나 만료된 링크입니다.')
    ...
    await logTokenAccess(tokenRow.id, 'view', ip, ua).catch((err) => {
      apiLogger.warn({ err, tokenId: tokenRow.id }, 'Failed to log token access')
    })
