...
export function TransactionTable({ transactions, onDelete }: TransactionTableProps) {
  ...
  const exportHeaders = ['거래번호', '날짜', '계약', '거래상대방', '유형', '금액', '통화', '상태']
  const exportRows = transactions.map((tx) => [
    tx.transaction_no,
    format(new Date(tx.transaction_date), 'yyyy-MM-dd'),
    tx.contract?.contract_no ?? '',
    tx.counterparty?.company_name_ko ?? '',
    TX_TYPE_LABELS[tx.transaction_type] ?? tx.transaction_type,
    tx.direction === 'payable' ? -tx.amount_original : tx.amount_original,
    tx.currency_code,
    tx.status,
  ])
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-end">
        <TableExportButton headers={exportHeaders} rows={exportRows} filename="거래내역" />
      </div>
      <Table>
        <TableHeader>
          <TableRow>
